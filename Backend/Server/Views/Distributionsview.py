        ...
        try:
            ...
            with db.session.begin_nested():
                ...
                for transfer in transfers:
                    ...
                    # Create a transfer record with total_cost
                    ...
                    # Create an Expense record for the transfer
                    expense = Expenses(
                        shop_id=shop_id,
                        item=inventory_item.itemname,
                        description=f"Stock distribution of {qty} {inventory_item.metric} {inventory_item.itemname}",
                        quantity=qty,
                        totalPrice=total_cost,
                        amountPaid=total_cost,        # Assuming full payment during distribution
                        created_at=datetime.utcnow(),
                        user_id=get_jwt_identity()
                    )
                    db.session.add(expense)
                    db.session.flush()  # To get expense_id

                    # Link the transfer to the expense
                    transfer_record.expense_id = expense.expense_id
                    db.session.add(transfer_record)

                    # Include total_cost and unit_price in the response

...
const CreditorActions = ({ creditor, onCreditorUpdated, onCreditorDeleted, shops = [] }) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`/api/diraja/creditors/${creditor.id}`, formData, {
        headers: {
  ...
  const handleDeleteConfirm = async () => {
    if (!window.confirm('Are you sure you want to delete this creditor? This action cannot be undone.')) return;

    setLoading(true);
    setMessage('');

    try {
      const response = await axios.delete(`/api/diraja/creditors/${creditor.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
        },
      });
    ...
    <div style={{ maxWidth: '600px', margin: 'auto' }}>
      ...
      <form onSubmit={handleEditSubmit}>
        ...
        <div style={{ marginBottom: '15px' }}>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Creditor Name:</label>
          <input
            type="text"
            name="name"
            value={formData.name}
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          {shops.length > 0 ? (
            ...
            <div style={{ fontSize: '13px', color: '#666' }}>No shops available.</div>
          )}
        </div>

        {/* Total Credit */}
        <div style={{ marginBottom: '15px' }}>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Total Credit Limit:</label>
          <input
            type="text"
            name="total_credit"
        ...
        <div style={{ marginBottom: '20px' }}>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Current Credit Amount:</label>
          <input
            type="text"
            name="credit_amount"
        ...
        >
          <div style={{ fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Credit Summary:</div>
          <div style={{ fontSize: '12px', color: '#666' }}>
            <div>Available Credit: {formatCurrency(formData.total_credit - formData.credit_amount)}</div>
            <div>
              Utilization:{' '}
              {formData.total_credit > 0
                ? ((formData.credit_amount / formData.total_credit) * 100).toFixed(1)
                : 0}
              %
            </div>
          </div>

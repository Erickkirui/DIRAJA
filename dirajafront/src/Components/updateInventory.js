...
const UpdateInventory = ({ inventoryId, onClose, onUpdateSuccess }) => {
  ...
  useEffect(() => {
    const fetchInventoryDetails = async () => {
      try {
        ...
        const response = await axios.get(`https://kulima.co.ke/api/diraja/inventory/${inventoryId}`, {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`https://kulima.co.ke/api/diraja/inventory/${inventoryId}`, updatedData, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
